...
class UserLogin(Resource):
    ...
    def post(cls):
        ...
        if user and safe_str_cmp(user.password, hashlib.md5(user_data['password'].encode("utf")).hexdigest()):
            ...
            return {"access_token": access_token, "refresh_token": refresh_token, "type": "coworker", "id": user.coworker_id}, 200
        ...
        if user and safe_str_cmp(user.password, hashlib.md5(user_data['password'].encode("utf")).hexdigest()):
            ...
            return {"access_token": access_token, "refresh_token": refresh_token, "type": "manager", "id": user.manager_id}, 200
